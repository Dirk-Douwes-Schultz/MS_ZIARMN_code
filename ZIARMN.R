#code to run the ZIARMN model from Section 4 of the main text
...
dengeeCode <- nimbleCode({
  ...
  eta52 ~ dnorm(0,sd=5)
  eta53 ~ dnorm(0,sd=5)
  #these have to be very small but not 0
  eta62 <- .0000000001
  eta63 <- .0000000001
  eta72 <- .0000000001
  eta73 <- .0000000001
...
dengeeConf$removeSampler(c("eta02","eta03",
                           "eta82","eta83"))
dengeeConf$addSampler(target = c("eta02","eta82"),type = "AF_slice")
dengeeConf$addSampler(target = c("eta03","eta83"),type = "AF_slice")
...
dengeeConf$addMonitors(c("sigma_02","sigma_03","alpha02","alpha03","phi",
                         "sdz","sdc","rhozc","S2","S3","mlik","eta62","eta63",
                         "eta72","eta73"))
...
param_list <- c("malpha02","malpha03","sigma_02","sigma_03",
                ...
                "Prec[1, 1]","Prec[1, 2]",
                "Prec[2, 2]","eta02","eta03","eta12","eta13","eta22","eta23",
                "eta32","eta33","eta42","eta43","eta52","eta53",
                "eta82","eta83",
                "alpha82","alpha83")
...
plot(samples[,c("alpha02[19]","alpha03[19]")])
