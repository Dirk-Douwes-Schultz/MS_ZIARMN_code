#code to run the Zeng model from Section 4 of the main text
...
dengeeCode <- nimbleCode({
  ...
  alpha92 ~ dunif(min=0,max=1)
  alpha93 ~ dunif(min=0,max=1)
  alpha10 ~ dunif(min=-1,max=0)
  eta02 ~ dnorm(0,sd=5)
  eta03 ~ dnorm(0,sd=5)
...
dengeeConf$addSampler(target = c("eta02"),type = "slice")
dengeeConf$addSampler(target = c("eta03"),type = "slice")
