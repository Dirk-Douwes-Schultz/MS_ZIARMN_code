#code to run the MS-ZIARMN model from Section 4 of the main text with ICAR intercepts
...
adj_CAR <-  as.numeric(as.carAdjacency(NMatrix)$adj)
num_CAR <- as.numeric(rowSums(NMatrix))
...
dengeeConsts <- list(mT=52,N=160,total=total,psi=y,tmax=tmax,mtmax=mean(tmax),
                     ...
                     adj_CAR=adj_CAR,num_CAR=num_CAR,L_CAR=length(adj_CAR))
...
dengeeCode <- nimbleCode({
  ...
  for(i in 1:N){
    for(t in 2:mT){
      ...
      mu[1,i,t] <- alpha02[i]+malpha02+alpha22*(verde[i]-mverde)+
        alpha32*(favela[i]-mfavela)+
        alpha42*ids_p[i]+
        alpha52*(pop_dens[i]-mpop_dens)+alpha12*(tmax[i,t-1]-mtmax)+
      ...
      mu[2,i,t] <- alpha03[i]+malpha03+alpha23*(verde[i]-mverde)+
        alpha33*(favela[i]-mfavela)+
        alpha43*ids_p[i]+
        alpha53*(pop_dens[i]-mpop_dens)+alpha13*(tmax[i,t-1]-mtmax)+
  ...
  alpha02[1:N]~dcar_normal(adj=adj_CAR[1:L_CAR], num=num_CAR[1:N], tau=prec_02,zero_mean = 1)
  alpha03[1:N]~dcar_normal(adj=adj_CAR[1:L_CAR], num=num_CAR[1:N], tau=prec_03,zero_mean = 1)
...
#for storing filtered probs ect.
...
#you might want to remove phi as it will use alot of memory
...
param_list_alpha <- c(alpha02_list,alpha03_list)
gelman.diag(samples[,param_list_alpha])
#should all be greater than 1000
effectiveSize(samples[,param_list])

#check the space-time residuals if they were saved
...
#can do other areas as well, but it takes a long time to calculate for all areas
for(i in 1:3){
  print(i)
